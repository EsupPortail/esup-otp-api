...
tenant will have distincts secrets (for administrative access and user
authentication), and use distinct set of parameters, allowing a minimal level
of isolation and personalisation.
...
- access to tenant-management routes (ie, routes with /tenants prefix) requires
  global API password
- access to restricted routes (ie, routes with /protected or /admin prefix)
  requires tenant identifier, passed via a X-Tenant header, and tenant-specific
  API password
