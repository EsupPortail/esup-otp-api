...
exports.get_available_transports = function(req, res, next) {
    find_user(req, res, function(user) {
        var response = {};
        var result = {};
        if (user[global.properties.esup.mongodb.transport.mail]) result.mail = utils.cover_string(user[global.properties.esup.mysql.transport.mail], 4, 5);
        if (user[global.properties.esup.mongodb.transport.sms]) result.sms = utils.cover_string(user[global.properties.esup.mysql.transport.sms], 2, 2);
...
exports.send_sms = function(req, res, callback) {
    find_user(req, res, function(user) {
        console.log(user);
        if (typeof(callback) === "function") callback(user[global.properties.esup.mysql.transport.sms]);
    });
}
...
exports.send_mail = function(req, res, callback) {
    find_user(req, res, function(user) {
        if (typeof(callback) === "function") callback(user[global.properties.esup.mysql.transport.mail]);
    });
}

exports.update_transport = function(req, res, next) {
    connection.query(
        "UPDATE " + global.properties.esup.mysql.userTable + " SET " + global.properties.esup.mongodb.transport[req.params.transport] + " = ? Where uid = ?", [req.params.new_transport, req.params.uid],
        function(err, result) {
            if (err) throw err;
            else res.send({
                code: 'Ok',
...
exports.delete_transport = function(req, res, next) {
    connection.query(
        "UPDATE " + global.properties.esup.mysql.userTable + " SET " + global.properties.esup.mongodb.transport[req.params.transport] + " = ? Where uid = ?", ["", req.params.uid],
        function(err, result) {
            if (err) throw err;
            else res.send({
                code: 'Ok',
